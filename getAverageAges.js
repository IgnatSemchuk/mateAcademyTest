...
function calculateMenAverageAge(people, century) {
  const mansAgeArr = people
    .filter(person => (person.sex === 'm') && !(century - Math.ceil(person.died / 100)))
    .map(man => getAge(man));

  return getAverageAge(mansAgeArr);
}
...
function calculateWomenAverageAge(people, withChildren) {
  const getWomenAge = people => people
    .filter(person => person.sex === 'f')
    .map(woman => getAge(woman));

  const getWomenWithChildrenAge = (people) => {
    return people.map(child => getMother(child, people))
      .filter((mother, index, mothersList) => !!mother && mothersList.indexOf(mother) === index)
      .map(woman => getAge(woman))
    };
    
  const womenAgeArr = !withChildren ? getWomenAge(people) : getWomenWithChildrenAge(people);
    
  return getAverageAge(womenAgeArr);
}
...
function calculateAverageAgeDiff(people, onlyWithSon) {
  const mothersChildrenAgeDiff = people
    .map(child => {
      const mother = !!child.mother && onlyWithSon
        ? child.sex === 'm' && getMother(child, people)
        : getMother(child, people);

      return !mother ? null : child.born - mother.born;
    })
    .filter(age => !!age);

  return getAverageAge(mothersChildrenAgeDiff);
}
...
function getMother(child, people) {
  return people.find(person => person.name === child.mother);
}

function getAverageAge(ageArr) {
  return ageArr.reduce((ageSum, age) => ageSum + age) / ageArr.length;
}
